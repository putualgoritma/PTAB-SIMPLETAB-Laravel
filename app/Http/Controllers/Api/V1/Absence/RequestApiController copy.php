    ...
    public function menuAdmin(Request $request)
    {
        ...
        if (in_array('absence_all_access', $checker)) {
            ...
            $shift_change = ShiftChange::selectRaw('count(shift_changes.id) as total')
                ...
                ->join('shift_planner_staffs as s1', function ($join) {
                    $join->whereDate('s1.start', '>=', date('Y-m-d'));
                    $join->on('s1.id', '=', 'shift_changes.staff_id');
                })
                ...
                ->join('shift_planner_staffs as s2', function ($join) {
                    $join->whereDate('s2.start', '>=', date('Y-m-d'));
                    $join->on('s2.id', '=', 'shift_changes.staff_id');
                })
                ->join('staffs as st2', 'st2.id', '=', 'shift_changes.staff_change_id')
                ->join('shift_groups as sh2', 'sh2.id', '=', 's2.shift_group_id')
                // ->whereDate('st1.start', '>=', date('Y-m-d'))
                // ->whereDate('st2.start', '>=', date('Y-m-d'))
