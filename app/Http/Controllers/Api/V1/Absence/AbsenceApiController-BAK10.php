...
class AbsenceApiController extends Controller
{
    ...
    public function schedule(Request $request)
    {
        ...
        $type = $coordinat->type;
        // if ($coordinat->type == "shift") {
        $list_absence = ShiftGroups::selectRaw('duration, duration_exp, type, shift_group_timesheets.time, shift_group_timesheets.start, absence_category_id,shift_group_timesheets.id as shift_group_timesheet_id ')
            ->join('shift_group_timesheets', 'shift_group_timesheets.shift_group_id', '=', 'shift_groups.id')
            ->join('shift_planner_staffs', 'shift_planner_staffs.shift_group_id', '=', 'shift_groups.id')
            ->join('absence_categories', 'shift_group_timesheets.absence_category_id', '=', 'absence_categories.id')
            ->where('staff_id', '323')
            ->orderBy('shift_planner_staffs.created_at', 'ASC')
            ->get();
