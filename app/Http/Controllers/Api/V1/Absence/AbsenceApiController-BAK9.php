...
class AbsenceApiController extends Controller
{
    ...
    {
        $data = [];
        $absence = Absence::join('days', 'days.id', '=', 'absences.day_id')->selectRaw('absences.id,DATE(created_at) as created_at, days.name as day_name')->where('staff_id', '323')->groupByRaw('DATE(created_at)')->get();
