...
class AbsenceController extends Controller
{
    ...
    public function reportAbsenceExcel(Request $request)
    {
        ...
        try {
            ...
            $staffs = Staff::select('staffs.*', DB::raw('RIGHT(staffs.NIK , 3 ) AS NIK'), 'work_types.type as work_type')
    ...
    public function reportAbsence(Request $request)
    {
        ...
        //     DB::raw('DATE(absences.created_at)'),
        ...
        //     ->join('shift_planner_staffs', 'shift_planner_staffs.id', '=', 'absence_logs.shift_planner_id')
        //     ->join('staffs', 'staffs.id', '=', 'absences.staff_id')
        ...
        try {
            ...
            $data12 = [];
            $stfs1 = Staff::select('staffs.*', DB::raw('RIGHT(staffs.NIK , 3 ) AS NIK'), 'work_types.type as work_type')
                ->join('work_types', 'work_types.id', '=', 'staffs.work_type_id')
                ->join('users', 'users.staff_id', '=', 'staffs.id')
                ->groupBy('staffs.id')
                ->orderBy('NIK', 'ASC')
                // ->where('staffs.id', '8')
                // ->where('work_types.type', 'shift')
                ->get();
            $ii = 0;

            foreach ($stfs1 as $stf) {

                if ($stf->work_type == "shift") {
                    $report = AbsenceLog::select(
                        DB::raw('RIGHT(staffs.NIK , 3 ) AS NIK'),
                        ...
                        'staffs.id as staff_id',
                        'subdapertements.name as subdapertement_name',
                        'absence_logs.shift_planner_id as shift_id'
