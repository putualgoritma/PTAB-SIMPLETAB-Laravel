...
class AdditionalTimeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // abort_unless(\Gate::allows('additionalTime_access'), 403);
        $checker = [];
        ...
        if (!in_array('absence_all_access', $checker)) {
            $qry = AbsenceRequest::selectRaw('absence_requests.*, staffs.name as staff_name')
                ...
                ->where('absence_requests.category', 'AdditionalTime')
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('actions', function ($row) {
                ...
                $editGate = '';
                $deleteGate = 'additionalTime_delete';
                $crudRoutePart = 'additionalTime';
        ...
        return view('admin.additionalTime.index');
    }
    public function create(Request $request)
    {
        // abort_unless(\Gate::allows('additionalTime_create'), 403);
        $type = $request->type;
        $staffs = Staff::orderBy('name')->get();
        return view('admin.additionalTime.create', compact('staffs', 'type'));
    }

    public function show($id)
    {
        // abort_unless(\Gate::allows('additionalTime_access'), 403);
        $additionalTime = AbsenceRequest::selectRaw('absence_requests.*, staffs.name as staff_name')
            ->join('staffs', 'staffs.id', '=', 'absence_requests.staff_id')
            ->where('absence_requests.id', $id)->first();

        $file = AbsenceRequestLogs::where('absence_request_id', $id)->get();

        // dd($additionalTime, $file);
        return view('admin.additionalTime.show', compact('additionalTime', 'file'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $additionalTime = AbsenceRequest::create($data);
        return redirect()->route('admin.additionalTime.index');
    }

    public function edit($id)
    {
        // abort_unless(\Gate::allows('additionalTime_edit'), 403);
        $requests = Requests::where('id', $id)->first();
        $type = $requests->type;
        $users = User::where('staff_id', '!=', '0')->orderBy('name')->get();
        return view('admin.additionalTime.edit', compact('users', 'type', 'requests'));
    }
    public function update(Request $request, $id)
    {
        // abort_unless(\Gate::allows('additionalTime_edit'), 403);
        $additionalTime = Requests::where('id', $id)
    ...
    {
        // abort_unless(\Gate::allows('additionalTime_delete'), 403);
        $d = AbsenceRequest::where('id', $id)
