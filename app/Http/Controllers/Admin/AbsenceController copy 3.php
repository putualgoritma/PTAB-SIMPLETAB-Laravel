...
class AbsenceController extends Controller
{
    ...
    public function reportAbsence(Request $request)
    {
        ...
        // dd($from, $to);
        try {
            // $awal = date("Y-m-d", strtotime('-1 month', strtotime($request->monthyear . '-21')));
            // $akhir  = date("Y-m-d", strtotime($request->monthyear . '-20'));
            // $from = date("Y-m-d", strtotime($request->from));
            // $to   = date("Y-m-d", strtotime($request->to));
            ...
            // dd(AbsenceLog::where('staff_id', '25')
            //     ->where('absence_category_id', '1')
            //     ->join('absences', 'absences.id', '=', 'absence_logs.absence_id')->get());
