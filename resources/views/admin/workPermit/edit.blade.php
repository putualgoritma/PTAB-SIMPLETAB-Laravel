    ...
    <div class="card-body">
        <form action="{{ route("admin.workPermit.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                <label for="date">{{ trans('global.workPermit.fields.date') }}*</label>
                <input type="date" id="date" name="date" class="form-control" value="{{ old('date', isset($workPermit) ? $workPermit->date : '') }}" required>
                @if($errors->has('date'))
                    <em class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </em>
                ...
                <p class="helper-block">
                    {{ trans('global.workPermit.fields.date_helper') }}
                </p>
            </div>
